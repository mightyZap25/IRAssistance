...
async function run() {
    ...
    console.log(`Getting User token (impersonating ${testRecipient})...`);
    try {
        const userToken = await getJwt(serviceAccount, ['https://www.googleapis.com/auth/chat.spaces', 'https://www.googleapis.com/auth/chat.messages'], testRecipient);
        console.log("User token obtained!");
        
        console.log(`Setting up DM for Bot with ${testRecipient} via spaces:setup with User Auth...`);
        const setupRes = await fetch('https://chat.googleapis.com/v1/spaces:setup', {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${userToken}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({
                space: { spaceType: "DIRECT_MESSAGE" },
                memberships: [{ member: { name: `users/app`, type: "BOT" } }]
            })
        });
        ...
        const msgRes = await fetch(`https://chat.googleapis.com/v1/${setupData.name}/messages`, {
            ...
            body: JSON.stringify({ text: "Test message from the Bot using User Auth setup!" })
        });
        const msgData = await msgRes.json();
        console.log(msgData.error ? `Message failed: ${JSON.stringify(msgData.error)}` : "Message sent successfully!");
    } catch (e) {
        console.error(e);
    }
}
