...
export default function LeaveManagementPage() {
    ...
    // Odoo 근태 연동
    const fetchOdooAttendance = async () => {
        if (!currentUser?.email) return;
        try {
            const res = await fetch(`http://localhost:5050/api/odoo/attendance/today?email=${encodeURIComponent(currentUser.email)}`);
            if (res.ok) {
                const data = await res.json();
                if (data) {
                    setAttendanceLog({
                        checkIn: data.check_in + 'Z', // Odoo returns UTC string
                        checkOut: data.check_out ? data.check_out + 'Z' : null,
                        odooId: data.id
                    });
                }
            }
        } catch (err) {
            console.error("Failed to fetch Odoo attendance:", err);
        }
    };
    ...
    useEffect(() => {
        let itv;
        const getDateObj = (val) => new Date(val); // UTC to local is handled automatically by Date constructor
    ...
    const handleCheckIn = async () => {
        try {
            const res = await fetch('http://localhost:5050/api/odoo/attendance/check-in', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: currentUser.email })
            });
            if (!res.ok) throw new Error(await res.text());
            await fetchOdooAttendance();
            alert('출근 등록되었습니다 (Odoo 연동됨).');
        } catch(err) {
            alert('출근 등록 실패: ' + err.message);
        }
    };

    const handleCheckOut = async () => {
        if (!window.confirm('정말 퇴근하시겠습니까? 퇴근 처리 후에는 취소할 수 없습니다.')) return;
        try {
            const res = await fetch('http://localhost:5050/api/odoo/attendance/check-out', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email: currentUser.email })
            });
            if (!res.ok) throw new Error(await res.text());
