...
const ECOPage = () => {
    ...
    const handleApprove = async (ecn) => {
        ...
        try {
            ...
            try {
                if (isFinalStep) {
                    await createNotificationByRoute('/parts', 'ECO 최종 승인 완료', `ECO [${ecn.ECNNumber || ecn.id}] 설계변경 건이 최종 승인되었습니다.`);
                } else {
                    await createNotificationByRoute('/eco', 'ECO 결재 승인', `ECO [${ecn.ECNNumber || ecn.id}] 건이 ${stepsList[currentStepIdx].label} 단계를 통과하였습니다.`);
                }
            } catch (notiErr) {
                console.warn("Failed to send ECO approval notification:", notiErr);
            }
    ...
    const handleReject = async (ecn) => {
        ...
        try {
            ...
            // 반려 알림 발송
            try {
                await createNotificationByRoute('/eco', 'ECO 결재 반려', `ECO [${ecn.ECNNumber || ecn.id}] 건이 반려 처리되었습니다. 사유: ${reason}`);
            } catch (notiErr) {
                console.warn("Failed to send ECO rejection notification:", notiErr);
            }
                    ...
                    <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md rounded-2xl border border-slate-200/50 dark:border-slate-800/80 shadow-sm flex-1 flex flex-col min-h-0 relative z-20 overflow-hidden mt-3 mx-1">
                        <MasterDataGrid
                            ...
                            cellRenderer={{
                                ...
                                ECNType: (val) => <span className="px-1.5 py-0.5 bg-slate-100 text-slate-600 text-[9px] font-black rounded uppercase tracking-widest border border-slate-200 shrink-0">{val || 'ECO'}</span>,
                                CurrentStep: (val, row) => (
                                    ...
                                    <div className="flex items-center justify-between gap-4 w-full pl-2">
                                        <div className="flex items-center gap-3 flex-[1.5] min-w-0">
                                            ...
                                            <div className="flex items-center gap-3 min-w-0 truncate">
                                                ...
                                                <span className="px-1.5 py-0.5 bg-slate-100 text-slate-600 text-[9px] font-black rounded uppercase tracking-widest border border-slate-200 shrink-0">{ecn.ECNNumber || 'ECO'}</span>
                                                <h3 className="text-sm font-black text-slate-900 truncate shrink max-w-[250px]">{ecn.Title || 'ECO Request'}</h3>
                        ...
                        <div className="flex justify-between items-center p-6 border-b border-slate-100 shrink-0">
                            <div>
                                <h2 className="text-lg font-black text-slate-800 flex items-center gap-2">
                                    <ClipboardList size={22} className="text-indigo-600" /> 설계변경(ECO) 승인서 작성 기안
                                </h2>
